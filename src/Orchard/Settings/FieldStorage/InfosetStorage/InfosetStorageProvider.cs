...
namespace Orchard.ContentManagement.FieldStorage.InfosetStorage {
    public class InfosetStorageProvider : IFieldStorageProvider {
        ...
        public IFieldStorage BindStorage(ContentPart contentPart, string fieldName) {
            var partName = XmlConvert.EncodeLocalName(contentPart.GetType().Name);
            fieldName = XmlConvert.EncodeLocalName(fieldName);
