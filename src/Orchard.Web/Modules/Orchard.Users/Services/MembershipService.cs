...
namespace Orchard.Users.Services {
    ...
    public class MembershipService : IMembershipService {
        ...
        public IUser GetUser(string username) {
            ...
            return _userRepository.Table.Where(u => u.NormalizedUserName == lowerName).FirstOrDefault();
        }

        public IUser ValidateUser(string userNameOrEmail, string password) {
            var lowerName = userNameOrEmail == null ? "" : userNameOrEmail.ToLowerInvariant();

            var user = _userRepository.Table.Where(u => u.NormalizedUserName == lowerName).FirstOrDefault();

            if (user == null)
                user = _userRepository.Table.Where(u => u.Email == lowerName).FirstOrDefault();
        ...
        public void SetPassword(IUser user, string password) {
            if (!(user is UserPartRecord))
                throw new InvalidCastException();

            var userPart = (UserPartRecord)user;
        ...
        private bool ValidatePassword(UserPartRecord userPart, string password) {
        ...
        private static void SetPasswordClear(UserPartRecord userPart, string password) {
            userPart.PasswordFormat = MembershipPasswordFormat.Clear;
            userPart.Password = password;
            userPart.PasswordSalt = null;
        }

        private static bool ValidatePasswordClear(UserPartRecord userPart, string password) {
            return userPart.Password == password;
        }

        private static void SetPasswordHashed(UserPartRecord userPart, string password) {
        ...
        private bool ValidatePasswordHashed(UserPartRecord userPart, string password) {
        ...
        private void SetPasswordEncrypted(UserPartRecord userPart, string password) {
        ...
        private bool ValidatePasswordEncrypted(UserPartRecord userPart, string password) {
